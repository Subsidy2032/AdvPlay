...
class FGSMEvasionVisualizer(BaseVisualizer,
                            attack_type=available_attacks.EVASION,
                            attack_subtype=evasion_techniques.FGSM):
    def visualize(self, context: FGSMEvasionVisualizationContext):
        out_dir = paths.VISUALIZATIONS_RESULTS / available_attacks.EVASION / \
            f"fgsm_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
        out_dir.mkdir(parents=True, exist_ok=True)

        self._plot_example(context, out_dir)

        print(f"FGSM visualization results saved under {out_dir}")
