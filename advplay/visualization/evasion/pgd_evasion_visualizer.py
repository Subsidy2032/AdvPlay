...
from advplay.visualization.contexts.pgd_evasion_visualization_context import PGDEvasionVisualizationContext
...
class PGDEvasionVisualizer(BaseVisualizer,
                           attack_type=available_attacks.EVASION,
                           attack_subtype=evasion_techniques.PGD):
    def visualize(self, context: PGDEvasionVisualizationContext):
        out_dir = paths.VISUALIZATIONS_RESULTS / available_attacks.EVASION / \
            f"pgd_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
        out_dir.mkdir(parents=True, exist_ok=True)

        self._plot_example(context, out_dir)

        print(f"PGD visualization results saved under {out_dir}")
...
def _as_displayable_image(x):
    ...
    if arr.ndim == 3 and arr.shape[-1] == 1:
        arr = arr.squeeze(-1)
    # Denormalizers hand images back in pixel space ([0, 255]); matplotlib reads float RGB as
    # [0, 1] and clips anything above 1 to white, so scale pixel-range images down for display.
