...
from advplay.visualization.contexts.c_w_evasion_visualization_context import CWEvasionVisualizationContext
...
class CWEvasionVisualizer(BaseVisualizer,
                          attack_type=available_attacks.EVASION,
                          attack_subtype=evasion_techniques.C_W):
    def visualize(self, context: CWEvasionVisualizationContext):
        out_dir = paths.VISUALIZATIONS_RESULTS / available_attacks.EVASION / \
            f"c_w_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
        out_dir.mkdir(parents=True, exist_ok=True)

        self._plot_example(context, out_dir)

        print(f"C&W visualization results saved under {out_dir}")
